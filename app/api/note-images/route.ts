...
const log = createLogger("api/note-images");
...
// POST /api/note-images  body: multipart/form-data, field "file"
export async function POST(req: Request) {
  ...
  try {
    ...
    mkdirSync(NOTE_IMAGES_DIR, { recursive: true });
    writeFileSync(join(NOTE_IMAGES_DIR, filename), buffer);

    log.info("note image uploaded", {
      filename,
      size: file.size,
      mime: file.type,
      durationMs: elapsedMs(startedAt),
    });
    return NextResponse.json({ url: `/api/note-images/${filename}`, filename });
  } catch (error) {
    log.error("note image upload failed", { error, durationMs: elapsedMs(startedAt) });
